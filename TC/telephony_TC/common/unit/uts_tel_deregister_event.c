...
#include "tet_macro.h"
#include "uts_tel_deregister_event.h"
...
struct tet_testlist tet_testlist[] = {
	{utc_tel_deregister_event_01, 1},
	{utc_tel_deregister_event_02, 2},
	{NULL, 0}
};
...
static void startup()
{
	...
	{
		if (TAPI_API_SUCCESS == tel_register_event(TAPI_EVENT_CLASS_DATA,&subscription_id,(TelAppCallback)register_app_callback,NULL))
...
void utc_tel_deregister_event_01()
{

	TapiResult_t  returnStatus = TAPI_API_SUCCESS;

	returnStatus = tel_deregister_event(subscription_id);
...
void utc_tel_deregister_event_02()
{

	TapiResult_t  returnStatus = TAPI_API_SUCCESS;

	returnStatus = tel_deregister_event(NULL);

	tet_printf ("The return value is %d\n", returnStatus);

	TET_CHECK_EQUAL(returnStatus,TAPI_API_INVALID_INPUT);
