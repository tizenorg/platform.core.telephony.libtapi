...
#include "uts_tel_release_call_all_held.h"
...
/**
TapiResult_t	tel_release_call_all_held (int *pRequestId);
*/


/**
 * @brief This tests tel_release_call_all_held() API in normal conditions (MO call)
 * @par ID:
 * UTC_TEL_RELEASE_CALL_ALL_01
 * @param [out] pRequestId = &RequestId
* @return TAPI_API_SUCCESS
*/
void utc_tel_release_call_all_held_01()
{
	CALL_TEST_DEBUG("\n################     TEST START    ################\n");
	TapiResult_t  ret_status ;
	...
	TET_CHECK(TAPI_API_SUCCESS, ret_status);
	expected_event = TAPI_EVENT_CALL_RELEASE_ALL_HELD_CNF;
	gmain_loop_for_tapi_events(TIME_DELAY1);
...
void utc_tel_release_call_all_held_02()
{
	CALL_TEST_DEBUG("\n########     TEST START    ###########\n");
	TapiResult_t  ret_status ;

	ret_status = tel_release_call_all_held (NULL);

	TET_CHECK(TAPI_API_INVALID_PTR, ret_status);

	tet_infoline("utc_tel_release_call_all_held_func_02");
	tet_result(TET_PASS);
