...
/**
* @ingroup  UTS_SIM_SAT
* @addtogroup UTS_TEL_ENABLE_PERSONALISATION tel_enable_sim_personalization
*
...
* 	The information required to enable network personalisation.
*	@code
	typedef struct
	{
...
/**
* @file		uts_tel_enable_sim_personalization.c
* @brief
...
#include "uts_tel_enable_sim_personalization.h"
...
{
	TET_STAMP_START("utc_tel_enable_sim_personalization01 -TAPI_API_INVALID_PTR ");
	TET_CHECK(TAPI_API_INVALID_PTR,tel_enable_sim_personalization (NULL, NULL));
	TET_STAMP_PASS();
	return;
}
...
{
	TET_STAMP_START("utc_tel_enable_sim_personalization02 ");
	int pRequestId=0;

	TelSimPersPw_t lock_pers = {0,};
	lock_pers.type = TAPI_SIM_PERS_NET;
	lock_pers.pw_len =	15;
	lock_pers.pw = (unsigned char*)malloc(15);
	memcpy(lock_pers.pw,"000000000000000",15);

	gAsyncResult = FALSE;
					
	TET_CHECK(TAPI_API_SUCCESS,tel_enable_sim_personalization(&lock_pers, &pRequestId));
